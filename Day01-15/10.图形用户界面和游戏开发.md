...
def main():
    ...
    # 创建顶层窗口，“top = tkinter.Tk()”创建了一个“Tk”类的实例，它表示图形用户界面 (GUI) 应用程序的主窗口或顶级窗口
    top = tkinter.Tk()
    # 设置窗口大小
    ...
    top.title('小游戏')
    # 创建标签对象并添加到顶层窗口
    #标签控件（Label）指定的窗口中显示的文本和图像,如果需要显示一行或多行文本且不允许用户修改，可以使用 Label 组件
    #`top`：它指定父窗口小部件，在本例中是顶级窗口（`Tk` 对象）。 标签将放置在该窗口内。
